/**
 * AI Quota Monitor - pywebview Injection Script
 *
 * Adapted from ai-monitor-android.js for use with pywebview (Windows/macOS).
 * Data is passed via window.pywebview.api.post_data(source, json)
 * instead of AndroidBridge.postData().
 *
 * Injected by WebviewFetcher after page load via window.events.loaded.
 */
(function () {
    'use strict';
    ...
    function send(source, data) {
        if (window.pywebview && window.pywebview.api) {
            window.pywebview.api.post_data(source, JSON.stringify(data));
        }
    }
    ...
    // ── claude.ai oclaw: auto-set hash and reload ───
    if (PAGE.key === 'claude_usage') {
        if (location.pathname === '/new' && !location.hash.includes('settings/usage')) {
