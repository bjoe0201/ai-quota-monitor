...
(function () {
    ...
    //  DEFERRED BOOT
    // ─────────────────────────────────────────────
    //  Tampermonkey 的 @grant 會注入 inspector.js 攔截頁面的 window.fetch()，
    //  在 React hydration 密集呼叫 fetch 的同時若本腳本也在操作 DOM，會互搶主執行緒導致卡頓。
    //  解法：把所有 DOM 操作延到 requestIdleCallback，等瀏覽器完成初始載入後再啟動。
    //  純資料的 PAGE_MAP / config / state 則保持同步初始化（不碰 DOM，不會卡）。
    // ─────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────
    //  延遲啟動：等瀏覽器空閒後才注入 UI / 啟動 timer
    //  避免在 Tampermonkey inspector.js 攔截 fetch 的高峰期操作 DOM
    // ─────────────────────────────────────────────
    const _deferBoot = typeof requestIdleCallback === 'function'
        ? (fn) => requestIdleCallback(fn, { timeout: 3000 })
        : (fn) => setTimeout(fn, 50);
    _deferBoot(_bootUI);
    return; // ← IIFE 到這裡結束同步執行，以下全在 callback 內
