...
| `ai-monitor-client-v4.4.js` | Tampermonkey 使用者腳本（v4.4.3 = v4.4.2 + claude.ai 新路由相容）。OpenAI / Claude.ai / Claude API / GitHub Copilot 透過 fetch / XHR hook 攔截 JSON API；**OpenRouter 因採 Next.js SSR + RSC 字串流（無 JSON API）改用 DOM 解析**：`/settings/credits` 優先讀容器 `aria-label="Remaining credits: X.XXX"`，fallback 至翻頁動畫；`/activity` 直接讀 textContent。**GitHub Budgets**（v4.4.2 新增）：在 `github.com/settings/billing/budgets` 解析 "All Premium Request SKUs" 列，以 `[class*="LinkText"]` 抓取 `spent` / `budget` 金額並計算百分比。**claude.ai 新路由**（v4.4.3）：Usage 頁面從 `/settings/usage` 遷移至 `/new#settings/usage`，`isOnExpectedPage()` 改為比對 hash，fetch/XHR hook 加入早期退出防止干擾聊天主頁。頁面右下角有浮動 ⚡ 圓點顯示連線狀態 |
